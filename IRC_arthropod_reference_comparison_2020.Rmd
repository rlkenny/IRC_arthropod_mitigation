...
css_tky1 <- arth_data %>% 
  filter(type == "CSS") %>% 
  select(year_restoration, evenness_functional)

css_tky1$year_restoration <- as.factor(css_tky1$year_restoration)

# Model the effect of habitat type on metric
model1 =lm(css_tky1$evenness_functional ~ css_tky1$year_restoration)
...
css_tky_bar_fe <- ggplot(css_tky1, aes(x = year_restoration, y = evenness_functional)) +
  geom_blank() +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  ...
  geom_boxplot(aes(fill=year_restoration), stat = "boxplot")+
  scale_fill_brewer(palette="Paired") +
  geom_text(data = labs_df1, aes(x = year_restoration, y = evenness_functional, label = Letters,vjust = -.5, hjust=-.5)) +
  theme(plot.title = element_text(vjust=-0.6))+
  scale_x_discrete()
...
# Model the effect of habitat type on metric
model1 =lm(gl_tky1$richness_taxa ~ gl_tky1$year_restoration)
...
gl_tky_bar_tr <- ggplot(gl_tky1, aes(x = year_restoration, y = richness_taxa)) +
  geom_blank() +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x = '\nYear of Restoration', y = 'Taxa richness (avg)') +
...
# Model the effect of habitat type on metric
model1 =lm(gl_tky1$evenness_functional ~ gl_tky1$year_restoration)
...
gl_tky_bar_fe <- ggplot(gl_tky1, aes(x = year_restoration, y = evenness_functional)) +
  geom_blank() +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x = '\nYear of Restoration', y = 'Functional evenness (avg)') +
